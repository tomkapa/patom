...
> Status: **IMPLEMENTED** (D0–D7) on branch `feat/discord-byo-integration` (PR #186) — the design below is the authoring plan; the **as-built deltas** are noted immediately after. Bringing Patom agents into Discord via customer-owned "bring-your-own" bots over Discord's **Gateway** (WebSocket) transport. Authored 2026-06-16. Discord facts re-verified against the official developer docs (`discord.com/developers/docs`) by an adversarial review pass; Patom facts re-verified against the shipped code. Companion to [`lark-byo-integration-plan.md`](lark-byo-integration-plan.md) — same product, same principles, different platform.
>
> **As-built deltas (2026-06-16, vs. the plan below):**
> - **Idempotency / dedup.** The planned dedicated `discord_message_id` column/side-map was **not** built. Mirroring dedups on `thread_messages.idempotency_key` with the key **`discord:{application_id}:{guild_id}:{message_id}`** — note it is **app-scoped** (not the plan's `discord:{guild}:{message}`), so two bots that share a guild don't dedupe each other's mirror into one thread.
> - **Thread binding key.** `discord_threads` is keyed by **`(org_id, application_id, guild_id, container_id)`**, not the plan's `(guild_id, container_id)` — bindings are scoped per bot/org so a shared guild/channel can't shadow across bots or tenants. `lookup_by_container` / `mark_backfilled` / the `bind` conflict target all carry `org_id`+`application_id`.
> - **Tables.** Shipped: `discord_apps` (+ `UNIQUE (org_id, agent_id)`), `discord_user_handles`, `discord_threads` (+ an `is_thread` column, migration 83), `discord_channels` (migration 82, for channel-membership mirroring). There is **no** `discord_message_id` table.
> - **Threaded replies (follow-up).** A top-level channel `@mention` now **opens a Discord thread** and the agent converses there; triggering stays strictly `@mention`-or-DM (a thread does **not** auto-continue). See [`discord-threaded-replies-plan.md`](discord-threaded-replies-plan.md). The §2/§5 worked examples below predate this.
> - **Trigger detection.** `mentions_bot` also scans raw `content` for the `<@id>`/`<@!id>` marker as a fallback to the `mentions` array. **Role mentions** (`<@&role_id>` of a role the bot holds) are **not yet** a trigger — tracked in issue #191.
> - **Concurrency.** Shadow-mint serializes first-sight per `(org, discord_user)` via `pg_advisory_xact_lock` (no orphan identities under a race).
> - **NOT yet built: D6.5 (Interactions + consent).** No `/patom link` slash command, `INTERACTION_CREATE` handling, or `/api/discord/identity/*` routes exist yet. The "consent" rungs in §6.5, §7, §9b, and the setup status ladder are still **planned**, not shipped. Admin surface is just `POST/GET/DELETE /api/discord/apps`.
>
