...
3. **Integration test.** Add coverage under `tests/`:
   ...
   - Per CLAUDE.md §3: failing test first. Confirm it fails for the expected reason before adding the migration.
...
6. **Mutating an existing catalog row in a follow-up migration without paired down.** CLAUDE.md §14. Catalog rows are append-or-update with explicit migrations; never edit migration 36 to add Gmail back into the seed list.
