...
- **Authoritative rules**: the repo's `CLAUDE.md` at the working-tree root, read fresh on every invocation. The `SPEC.md` it references may or may not exist — if it does, consult for data-model context, but never let its absence block the review.
...
1. Read `CLAUDE.md` in full. Don't summarize from memory; the file is short (~260 lines) and changes occasionally.
...
3. Note any project-specific conventions visible in the touched files' neighbors that aren't in CLAUDE.md — for example, a module's preferred naming pattern, its error-type layout, how it constructs its tracing spans. These count as conventions too.
...
### Phase 3 — Review against CLAUDE.md

For each changed Rust file, walk through the CLAUDE.md sections and check for violations. The checklist below is non-exhaustive — use it as a prompt, not a ceiling. When in doubt about whether something is a violation, read the relevant CLAUDE.md section verbatim again and decide.
...
**Front-end / TypeScript** — CLAUDE.md is Rust-only, but apply the spirit: no `any`, no magic strings for IDs that have a typed counterpart server-side, no silent type assertions.
...
- **CONFIRM**: the diff is fine — this category exists so the report can affirmatively note compliance for each major CLAUDE.md section that the diff touched, rather than only listing problems.
...
2. After all edits, run the relevant CLAUDE.md §3 gates on the touched code:
   - `cargo fmt --all -- --check`
...
### Not reviewed
- Front-end `.tsx` changes — CLAUDE.md is Rust-only; spot-checked for obvious issues, none found.
