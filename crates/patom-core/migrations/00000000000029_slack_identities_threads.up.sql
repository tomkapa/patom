...
--
-- `slack_identities` links a Slack user to a Patom user. Phase 1 leaves
...
-- (channel + thread_ts) and a Patom DAG (root_request_id). One row per
...
-- (regardless of which Patom org). This prevents a single Slack user
-- from being claimed by two Patom tenants and is the safety net for
...
CREATE TABLE slack_threads (
    ...
    -- The Patom DAG root. Every prompt_requests row whose
