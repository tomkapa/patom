...
struct Harness {
    ...
    agent_id: AgentId,
    state: AppState,
    /// Principal pinned to `org_id` so the seeded default agent
...
async fn lists_tool_calls_for_server_with_agent_name_and_error_message(pool: PgPool) {
    ...
    let state_id = seed_agent_thread_state(&pool, h.org_id, h.agent_id).await;
    let request =
        seed_prompt_request(&h.state.pool, state_id, h.agent_id, h.org_id, h.user_id).await;
